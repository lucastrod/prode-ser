...
export async function sendPasswordResetEmail(email: string, name: string, token: string) {
  ...
  if (!apiKey) {
    console.log('----------------------------------------------------');
    console.log(`⚠️  RESEND_API_KEY no configurada. SIMULANDO envío de RECUPERACIÓN.`);
    console.log(`✉️  Email para: ${email} (${name})`);
  ...
  const { error } = await resend.emails.send({
    from: 'PRODE SER <onboarding@resend.dev>',
    to: email,
    subject: 'Recuperación de Contraseña - PRODE SER 🏆',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h1 style="color: #1B199A; margin-bottom: 5px;">PRODE SER</h1>
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetLink}" style="background-color: #1B199A; color: #ffffff; padding: 12px 24px; text-decoration: none; font-weight: bold; border-radius: 9999px; box-shadow: 0 4px 12px rgba(27, 25, 154, 0.3); display: inline-block;">
            Cambiar Contraseña
          </a>
        </div>
        <p style="color: #9CA3AF; font-size: 12px; margin-top: 30px; border-top: 1px solid #f3f4f6; padding-top: 15px; text-align: center;">
